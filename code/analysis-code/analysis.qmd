...
################################################################################
# Keep variable names for predictors
formula <- as.formula("COVID ~ .")

# Explicitly remove rows with any NA in predictors (even if COVID is not NA)
train_data <- train_data %>% select(COVID, everything()) %>% drop_na()

# Now get model matrix
...
library(mice)        # For working with imputed data
library(tidyr)       # For data reshaping

# --- LASSO MODEL WITH CROSS-VALIDATION ON IMPUTED DATA ---

# Function to run Lasso with sample weights and calculate performance metrics
run_lasso_cv <- function(imp_data, outcome = "COVID", 
                        predictors = c("SLEEP", "DEPRESSED", "SOCIAL", "TROUBLE_SLEEPING", "EXERCISE"),
                        weight_var = "sampleweight") {
  
  # Extract data from current imputation
  X <- as.matrix(imp_data[, predictors])
  y <- imp_data[[outcome]]
  weights <- imp_data[[weight_var]]
  
  # Standardize predictors (important for Lasso)
  X_scaled <- scale(X)
  
  # Run cross-validated Lasso with sample weights
  ...
  # Get predictions
  pred_probs <- predict(final_model, newx = X_scaled, type = "response")
  
  # Calculate AUC
  roc_obj <- roc(y, pred_probs)
  auc_value <- auc(roc_obj)
  
  # Create calibration data for expected vs observed plot
  n_groups <- 10  # Number of bins for calibration plot
  
  # Create calibration data
  calib_data <- data.frame(
    predicted = as.vector(pred_probs),
    observed = y
  )
  
  # Sort by predicted probability and divide into bins
  calib_data <- calib_data[order(calib_data$predicted), ]
  calib_data$bin <- cut(seq(1, nrow(calib_data)), breaks = n_groups, labels = FALSE)
  
  # Calculate mean predicted and observed values for each bin
...
print(class(data_imputed))

# Initialize results containers
auc_values <- numeric(length = 5)
...
run_lasso_cv <- function(imp_data, outcome = "COVID", 
  ...
  # Extract data from current imputation and handle missing values
  # First, check for and handle missing data
  complete_cases <- complete.cases(imp_data[, c(outcome, predictors, weight_var)])
  if (!all(complete_cases)) {
    message("Removing ", sum(!complete_cases), " rows with missing values")
    imp_data <- imp_data[complete_cases, ]
  }
...
# Combine calibration results from all imputations
all_calibration <- do.call(rbind, calibration_results)

# ----------------------------
# OUTPUT RESULTS
# ----------------------------

# Print AUC values for each imputation
cat("\nAUC values for each imputation:\n")
print(auc_values)
cat("\nMean AUC across imputations:", mean(auc_values), "\n")

# Plot expected vs observed probabilities for all imputations
calibration_plot <- ggplot(all_calibration, aes(x = mean_predicted, y = mean_observed, color = factor(imputation))) +
  geom_point(size = 3) +
  geom_line() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") +
  labs(
    title = "Expected vs. Observed Probabilities",
    subtitle = paste("Mean AUC =", round(mean(auc_values), 3)),
    x = "Expected (Predicted Probability)",
    y = "Observed (Actual Proportion)",
    color = "Imputation"
  ) +
  theme_minimal() +
  coord_equal() +
...
# Print average coefficients across imputations
cat("\nAverage Lasso coefficients across imputations:\n")
# Extract coefficient names from the first imputation
coef_names <- rownames(coef_results[[1]])

# Create matrix to store coefficients from all imputations
all_coefs <- matrix(0, nrow = length(coef_names), ncol = 5)
rownames(all_coefs) <- coef_names

# Fill in the coefficient matrix
for (i in 1:5) {
  all_coefs[, i] <- as.vector(coef_results[[i]])
}

# Calculate mean coefficients
mean_coefs <- rowMeans(all_coefs)
...
non_zero_coefs <- coef_summary[abs(coef_summary$Mean_Coefficient) > 0.001, ]
print(non_zero_coefs[order(abs(non_zero_coefs$Mean_Coefficient), decreasing = TRUE), ])
```
...
calib_data_list <- vector("list", length(data_imputed))

for (i in seq_along(data_imputed)) {
  dat <- data_imputed[[i]] %>%
    drop_na(all_of(c(outcome_var, predictor_vars, "SAMPWEIGHT")))
  ...
  pred_probs <- predict(cv_fit, newx = X_test, s = "lambda.min", type = "response") %>% as.vector()

  # Count unique predicted probabilities
  n_unique_preds <- length(unique(pred_probs))

  # Only proceed with binning if there are at least 2 unique predicted probabilities
  if (n_unique_preds >= 2) {
    # Safely bin the predicted probabilities
    n_bins <- min(5, n_unique_preds)

    calib_data <- tibble(
      actual = y_test,
      predicted = pred_probs
    ) %>%
      mutate(bin = cut_number(predicted, n = n_bins, labels = FALSE)) %>%
      group_by(bin) %>%
      summarise(
        mean_pred = mean(predicted),
    ...
    calib_data_list[[i]] <- calib_data
  } else {
    message("Skipping calibration plot for imputation ", i, ": too few unique predicted values (", n_unique_preds, " unique values)")
  }
}

# Combine all calibration data
all_calib_data <- bind_rows(calib_data_list)

# Create calibration plot
ggplot(all_calib_data, aes(x = mean_pred, y = obs_rate, color = imputation)) +
  geom_line() +
  geom_point(size = 2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "gray40") +
  scale_x_continuous("Expected Probability", limits = c(0, 1), labels = percent_format(accuracy = 1)) +
  scale_y_continuous("Observed Proportion", limits = c(0, 1), labels = percent_format(accuracy = 1)) +
  labs(title = "Calibration Plot: Lasso Logistic Regression", color = "Imputation") +
  theme_minimal()
...
# Save the plot
ggsave(filename = here::here("results", "figures", "lasso_calibration_plot.png"),
       plot = calibration_plot_lasso,
       width = 8, height = 6, dpi = 300)

# Initialize list to store AUCs
auc_list <- vector("numeric", length(data_imputed))
...
for (i in seq_along(data_imputed)) {
  ...
  set.seed(1234 + i)
  train_idx <- sample(seq_len(nrow(X)), size = 0.7 * nrow(X))
  test_idx <- setdiff(seq_len(X), train_idx)
  ...
  # Compute AUC
  auc_val <- roc(y_test, pred_probs)$auc
  auc_list[i] <- auc_val
}
...
# Mean (pooled) AUC
cat("\nPooled AUC across imputations:", round(mean(auc_list), 3), "\n")
```
...
results_with_preds <- cross_validation_imputed_predictions(
  imputed_data = data_imputed,
  predictors = c("EXERCISE", "SLEEP", "DEPRESSED", "TROUBLE_SLEEPING", "SOCIAL")
)
...
Now, we will make expected versus observed plots.
```{r}
plot_calibration_multiple <- function(actual, predicted_list, n_bins = 10, title = "Calibration Plot") {
  # Combine calibration data across imputations
  calib_data_all <- map2_dfr(predicted_list, seq_along(predicted_list), function(predicted, i) {
...
# Example usage:
plot_cal <- plot_calibration_multiple(
  actual = test_data$COVID,
  predicted_list = pred_probs_list,  # this should be a list of 5 vectors
  title = "Random Forest Calibration Across 5 Imputations"
)
...
# Save it
ggsave(
  filename = here("results", "figures", "rf_calibration_all_imputations.png"),
  plot = plot_cal,
  width = 8,
  height = 6,
  dpi = 300
