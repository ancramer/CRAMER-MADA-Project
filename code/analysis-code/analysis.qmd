...
# Fit svyglm models for EXERCISE predictor
models_exercise <- lapply(survey_designs, function(dsgn) {
  svyglm(COVID ~ EXERCISE, design = dsgn, family = quasibinomial())
})

# Extract coefficients and standard errors for each model
model_exercise_coef <- lapply(models_exercise, function(model) {
  data.frame(
    Estimate = coef(model),
...
# Calculate means and variances for pooling
estimates <- do.call(rbind, lapply(model_exercise_coef, function(x) x$Estimate))
se <- do.call(rbind, lapply(model_exercise_coef, function(x) x$Std.Error))
...
# Create a data frame with pooled estimates, standard errors, and confidence intervals
pooled_exercise <- data.frame(
...
Now we will make a nice table of that using gt.
...
# Assuming 'pooled_exercise' is your data frame
pooled_exercise %>%
  group_by(Predictor) %>%
