---
title: "Learning Influenza Forecasting"
...
Data for this project is survey data collected from the National Health Interview Survey, produced by IPUMS through the University of Minnesota. This data was collected among United States residents of all ages and genders from 2019 - 2022 in the United States. There's 151406 observations and 47 vairables. 

Survey design variables were used and new binary variables were created from other variables. 
...
Raw data called NHIS_COVID.rds, sits with the data folder, in a a subfolder called raw data. The processed data, called data3.rds, is within the data folder in a subfolder called processed-data. The code to clean and explore and process this dadta is within the code folder, in the processing-code subfolder, called data-cleaning-project.qmd.

The raw data was loaded, and necessary variables and survery design variables were selected. After exploring each variable, new binary variables were created.

For now, let us load the processed data. Here's a histogram of the processed outcome vairable, which represents whether or not an indivudal has ever tested postive for COVID. Histograms of all vairables are available in the data-cleaning-project.qmd file, but will not be shown here for the sake of space.
```{r}
library(here)
data <- read_rds(here("data", "processed-data", "data3.rds"))
hist(data$COVID)
```
...
#| tbl-cap: "Data summary table, proportions of 0 compared to 1, and proportion NA to not NA."
#| echo: FALSE
here()
knitr::include_graphics(here("data","processed-data", "figures", "summary_table.png"))
```
